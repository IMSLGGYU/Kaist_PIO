# -*- coding: utf-8 -*-
"""
EQ1 IO Grid UI (PySide6)
- Tag 기반 Inputs/Outputs GRID
- Outputs Force: 여러개 동시 가능 (force override로 유지)
- PIO(Port side) 8채널(OHT/AMR S1/S2/S7/S8) 상태머신(LOAD/UNLOAD)
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("EQ1 Testbed IO (Grid / PySide6)")
        ...
        # ---- PIO 채널(EQ1: OHT/AMR S1/S2/S7/S8)
    ...
    def _build_pio_points(self):
        ...
        for st, byte in self.PIO_CHANNEL.items():
            for sig, bit, desc in in_list_oht:
            ...
            for sig, bit, desc in out_list_oht:
    ...
    def _init_ui(self):
        ...
        self.matrix_bytes = list(range(6, 17))
        gb_mat = QGroupBox("Raw IO Matrix (byte 6~16 × bit 0~7)  —  OUT 클릭=토글+Force(유지)")
    ...
    def reset_on_timeout(self):
        ...
        # PIO 채널: READY/L/U=0 + HO_AVBL/ES=1
        for m in self.pio.values():
            m.state = PIOState.IDLE
            m.mode = None
            m.reset_outputs()
