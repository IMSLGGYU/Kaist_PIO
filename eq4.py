# -*- coding: utf-8 -*-
"""
EQ4 IO Grid UI (PySide6)
- Tag 기반 Inputs/Outputs GRID
- Outputs Force: 여러개 동시 가능 (force override로 유지)
- PIO(Port side) 3채널(EQ4_P3, EQ4_P2, EQ4_P1) 상태머신(LOAD/UNLOAD)
...
try:
    ...
    from PySide2.QtWidgets import (
        ...
        QAbstractItemView, QSpinBox, QPlainTextEdit, QButtonGroup, QRadioButton, QComboBox
    )
except ImportError:
    # PySide2가 없으면 PySide6 시도
...
# ---- Roller library
from roller import RollerController, RollerIOMap, BitAddr


INFINITE = int(0xFFFFFFFF)

# =========================
# PIO Channel/Timeout Config (easy edit)
# =========================
PIO_TIMEOUTS = {
    "TP1": 240,
    "TP2": 240,
...
# =========================
# Roller / Carrier Address Map (easy edit)
# x.y 형태는 x=byte, y=bit
# - RollerController는 아래 4개 주소만 바꾸면 재사용 가능
#   out_run / out_dir / in_inward_stop / in_outward_stop
# - Carrier inputs는 (byte,bit) 리스트로 넣으면 됨 (여러개 가능)
# =========================

# Roller IOMap per channel
ROLLOUT = 1  # just a visual constant (unused)

ROLLER_IOMAPS: Dict[str, RollerIOMap] = {
    # EQ4_P1
    #  - 정방향(투입): 3.6=1, 3.7=0
    #  - 역방향(배출): 3.6=1, 3.7=1
    "EQ4_P1": RollerIOMap(
        out_run=BitAddr(byte=3, bit=6),
        out_dir=BitAddr(byte=3, bit=7),
        in_inward_stop=BitAddr(byte=3, bit=6),   # 진입(LOAD) 센서: 3.6
        in_outward_stop=BitAddr(byte=3, bit=7),  # 배출(UNLOAD/transfer) 센서: 3.7
    ),
    # EQ4_P2
    "EQ4_P2": RollerIOMap(
        out_run=BitAddr(byte=3, bit=4),
        out_dir=BitAddr(byte=3, bit=5),
        in_inward_stop=BitAddr(byte=3, bit=5),
        in_outward_stop=BitAddr(byte=3, bit=4),
    ),
    # EQ4_P3
    "EQ4_P3": RollerIOMap(
        out_run=BitAddr(byte=3, bit=2),
        out_dir=BitAddr(byte=3, bit=3),
        in_inward_stop=BitAddr(byte=3, bit=2),
        in_outward_stop=BitAddr(byte=3, bit=3),
    ),
}

# Carrier inputs per channel (edge detection용)
# - LOAD: rise(0->1) 우선
# - UNLOAD: fall(1->0) 우선
CARRIER_INPUTS: Dict[str, List[Tuple[int, int]]] = {
    "EQ4_P1": [(3, 6)],  # LOAD용 센서만 사용 (UNLOAD는 transfer_to_amr 완료로 OFF)
    "EQ4_P2": [(3, 4)],  # LOAD용 센서만 사용
    "EQ4_P3": [(3, 4)],  # LOAD용 센서만 사용
}



# =========================
# IO Point Definition (Python 3.6 호환: dataclass 대신 일반 클래스)
# =========================
class IOPoint:
    __slots__ = ("tag", "addr", "desc", "direction", "byte_index", "bit_index", "invert")

    def __init__(self, tag, addr, desc, direction, byte_index, bit_index, invert=False):
        object.__setattr__(self, "tag", tag)
        object.__setattr__(self, "addr", addr)
        object.__setattr__(self, "desc", desc)
        object.__setattr__(self, "direction", direction)
        object.__setattr__(self, "byte_index", byte_index)
        object.__setattr__(self, "bit_index", bit_index)
        object.__setattr__(self, "invert", invert)

    def __setattr__(self, name, value):
        raise AttributeError("IOPoint is immutable")
...
# =========================
class WMX3IoDriver:
    def __init__(self, device_name: str = "EQ4_IO_Grid"):
...
# - RollerController는 io.SetOutBit(port, byte, bit, value) / io.GetInBit(port, byte, bit) 형태를 기대함
# - 기존 WMX3IoDriver를 그대로 재사용하기 위한 어댑터
# =========================
...
class PIOPortMachine:
    def __init__(
        ...
        self._last_carrier_bits = None  # type: Optional[List[int]]

        # timeouts (sec)
        t = timeouts or {}
        self.TP1 = t.get("TP1", 240)
        self.TP2 = t.get("TP2", 240)
        self.TP3 = t.get("TP3", 240)
        self.TP4 = t.get("TP4", 240)
        self.TP5 = t.get("TP5", 240)
        self.WAIT_VALID_TIMEOUT = t.get("WAIT_VALID", 30)

        # 캐리어 엣지 없을 때 REQ 자동 OFF (기본 30초)
        self.REQ_FALLBACK_OFF = t.get("REQ_FALLBACK_OFF", 30)
        self._req_on_ts = None          # type: Optional[float]
        self._req_off_done = False
        self._unload_transfer_started = False
    ...
    def _timeout(self, limit: float) -> bool:
        return (time.time() - self.t0) > float(limit)
    ...
    # -------------------------
    # carrier edge (optional)
    # -------------------------
    def _carrier_edge(self, in_bytes: List[int]):
...
# -------------------------
    ...
    def tick(self, in_bytes: List[int]) -> Optional[str]:
        ...
        # ✅ REQ OFF rule (EQ1_P1과 동일):
        # 1순위) 캐리어 입력 엣지 발생 시 즉시 OFF
        #   - LOAD  : 0->1 (rise)
        #   - UNLOAD: 1->0 (fall)
        # 2순위) 엣지가 없으면 REQ ON 후 REQ_FALLBACK_OFF(기본 30초) 경과 시 OFF
        now = time.time()
        if self.mode in ("LOAD", "UNLOAD") and (self._req_on_ts is not None) and (not self._req_off_done):
            # ✅ LOAD: 3.6 상승(edge) 또는 fallback으로 L_REQ OFF
            if self.mode == "LOAD":
                if carrier_edge and carrier_edge[0]:
                    self.drv.set_out_bit(self.out_byte, PIOBits.L_REQ, 0)
                    ...
                    self._req_off_done = True
                    self._seq_mark(5)
            # ✅ UNLOAD: U_REQ OFF는 transfer_to_amr 종료 시점에만 수행
        ...
        if self.state == PIOState.WAIT_VALID:
            if valid == 1 and cs0 == 1:
                ...
                if self.mode == "LOAD":
                    ...
                    self._req_off_done = False
                    self.log(f"[{self.name}] L_REQ ON")
                else:
                    self.drv.set_out_bit(self.out_byte, PIOBits.U_REQ, 1)
                    self._req_on_ts = time.time()
                    self._req_off_done = False
                    self.log(f"[{self.name}] U_REQ ON")
            ...
            if tr == 1:
                ...
                if self.mode == "LOAD" and self.roller is not None and (not getattr(self, "_roller_load_started", False)):
                    ...
                    def _after_move_in(ok):
                        ...
                        self.log(f"[{self.name}] move_in done={ok} -> L_REQ OFF")
                    self._roller_thread(lambda: self.roller.move_in_until_sensor(timeout_s=10), after_fn=_after_move_in)
                # ✅ UNLOAD: READY ON 직후 transfer_to_amr 실행, 종료 시 U_REQ OFF
                if self.mode == "UNLOAD" and self.roller is not None and (not self._unload_transfer_started):
                    self._unload_transfer_started = True
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("EQ4 Testbed IO (Grid / PySide6)")
        ...
        # ---- Roller (EQ1_P1과 동일 시퀀스를 EQ4_P1~P3에도 적용)
        self._roller_io = DriverIoAdapter(self.driver)
        self.rollers: Dict[str, RollerController] = {}
        for ch in PIO_OUT_CHANNELS.keys():
            iomap = ROLLER_IOMAPS.get(ch, RollerIOMap())
            self.rollers[ch] = RollerController(io=self._roller_io, iomap=iomap)

        # ---- Carrier inputs (채널별 센서 주소)
        self.carrier_inputs = dict(CARRIER_INPUTS)

        # ---- PIO 채널(EQ4)
        # ⚠️ 아래 byte index는 실제 매핑에 맞게 수정해줘.
        self.PIO_IN_CHANNEL = dict(PIO_IN_CHANNELS)
        self.PIO_OUT_CHANNEL = dict(PIO_OUT_CHANNELS)
        self.matrix_bytes = list(MATRIX_BYTES)
        ...
        for name, out_b in self.PIO_OUT_CHANNEL.items():
            ...
            self.pio[name] = PIOPortMachine(
                ...
                driver=self.driver,
                log_fn=self.log,
                timeouts=PIO_TIMEOUTS,
                roller=self.rollers.get(name),
    ...
    def _build_pio_points(self):
        ...
        # ---- Roller IO points (표시용)
        for ch, iomap in ROLLER_IOMAPS.items():
            # Outputs
            self.out_points.append(IOPoint(
                tag=f"{ch}_ROLLER_RUN",
                addr=f"{ch}.ROLLER.OUT.RUN",
                desc="Roller RUN (ON/OFF)",
                direction="OUT",
                byte_index=iomap.out_run.byte,
                bit_index=iomap.out_run.bit
            ))
            self.out_points.append(IOPoint(
                tag=f"{ch}_ROLLER_DIR",
                addr=f"{ch}.ROLLER.OUT.DIR",
                desc="Roller DIR (0=inward, 1=outward)",
                direction="OUT",
                byte_index=iomap.out_dir.byte,
                bit_index=iomap.out_dir.bit
            ))
            # Inputs
            self.in_points.append(IOPoint(
                tag=f"{ch}_ROLLER_INWARD_STOP",
                addr=f"{ch}.ROLLER.IN.INWARD_STOP",
                desc="Inward stop sensor",
                direction="IN",
                byte_index=iomap.in_inward_stop.byte,
                bit_index=iomap.in_inward_stop.bit
            ))
            self.in_points.append(IOPoint(
                tag=f"{ch}_ROLLER_OUTWARD_STOP",
                addr=f"{ch}.ROLLER.IN.OUTWARD_STOP",
                desc="Outward stop sensor",
                direction="IN",
                byte_index=iomap.in_outward_stop.byte,
                bit_index=iomap.in_outward_stop.bit
            ))

        # ---- Carrier IO points (표시용)
        for ch, pairs in CARRIER_INPUTS.items():
            for idx2, (b, bt) in enumerate(pairs, start=1):
                tag = f"{ch}_CARRIER" if len(pairs) == 1 else f"{ch}_CARRIER_{idx2}"
                self.in_points.append(IOPoint(
                    tag=tag,
                    addr=f"{ch}.IN.CARRIER",
    ...
    def _init_ui(self):
        ...
        grid_label = f"Raw IO Matrix (byte {min_b}~{max_b} × bit 0~7)  —  OUT 클릭=토글+Force(유지)"

        # ===== Top Bar =====
        self.section_top = QWidget()
        top = QHBoxLayout(self.section_top)
        top.setContentsMargins(0, 0, 0, 0)
        main.addWidget(self.section_top)
        ...
        main.addWidget(self.section_mid, 1)
        ...
        # ===== IO Matrix Toggle =====
        toggle = QHBoxLayout()
        main.addLayout(toggle)
        self.btn_toggle_matrix = QPushButton(f"Show IO Grid (byte {min_b}~{max_b})")
        ...
        # ===== Raw Matrix =====
        self.gb_mat = QGroupBox(grid_label)
        self.gb_mat.setVisible(False)
        main.addWidget(self.gb_mat, 1)
        ...
        main.addWidget(self.section_bottom, 0)
    ...
    def _update_sequence_view(self):
