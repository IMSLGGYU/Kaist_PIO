...
- PIO(Port side) 2채널(EQ2_S5, EQ2_S6) 상태머신(LOAD/UNLOAD)
...
# Format: "EQ2_S5": (byte, bit)
# Example: "EQ2_S6": (2, 4)
CARRIER_INPUT = {
    "EQ2_S5": (2, 3),  # byte 2, bit 3
    "EQ2_S6": (3, 2),
}
...
class PIOPortMachine:
    def __init__(
        ...
        carrier_byte: Optional[int] = None,
        carrier_bit: Optional[int] = None
    ):
        self.name = name
        ...
        self.carrier_byte = carrier_byte
        self.carrier_bit = carrier_bit
    ...
    def _carrier_present(self, in_bytes: List[int]) -> Optional[int]:
        if self.carrier_byte is None or self.carrier_bit is None:
            return None
        return int((in_bytes[self.carrier_byte] >> self.carrier_bit) & 1)

    def _carrier_edge(self, current: Optional[int]) -> Optional[Tuple[int, int]]:
        if current is None:
            return None
        prev = self._last_carrier
        self._last_carrier = current
    ...
    def tick(self, in_bytes: List[int]) -> Optional[str]:
        if self.state == PIOState.IDLE:
            ...
            if self.auto_start:
                ...
                if valid == 1 and cs0 == 1:
                    self.reset_outputs()
                    carrier = self._carrier_present(in_bytes)
                    if carrier == 1:
                        self.mode = "UNLOAD"
                    ...
                    self.t0 = time.time()
                    self.log(f"[{self.name}] AUTO START {self.mode} -> WAIT_TR_REQ(TP1)")
            return None
        ...
        carrier = self._carrier_present(in_bytes)
        carrier_edge = self._carrier_edge(carrier)

        # REQ OFF rule (eq1_final? ??):
        # 1) ??? ?? ?? ? ?? OFF
        # 2) ??? ??? REQ ON ? REQ_FALLBACK_OFF ?? ? OFF
        now = time.time()
        if self.mode in ("LOAD", "UNLOAD") and (self._req_on_ts is not None) and (not self._req_off_done):
            if self.mode == "LOAD" and carrier_edge == (0, 1):
                self.drv.set_out_bit(self.out_byte, PIOBits.L_REQ, 0)
                self._req_off_done = True
                self._seq_mark(5)
            elif self.mode == "UNLOAD" and carrier_edge == (1, 0):
                self.drv.set_out_bit(self.out_byte, PIOBits.U_REQ, 0)
                self._req_off_done = True
                self._seq_mark(5)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        for name, out_b in self.PIO_OUT_CHANNEL.items():
            ...
            carrier_cfg = CARRIER_INPUT.get(name)
            self.pio[name] = PIOPortMachine(
                ...
                auto_start=AUTO_START_CHANNELS.get(name, False),
                carrier_byte=carrier_cfg[0] if carrier_cfg else None,
                carrier_bit=carrier_cfg[1] if carrier_cfg else None
            )
    ...
    def _add_extra_inputs(self):
        for st, (byte, bit) in CARRIER_INPUT.items():
            self.in_points.append(
                IOPoint(
                    tag=f"{st}_CARRIER",
